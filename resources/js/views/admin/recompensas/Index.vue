                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between pb-2 mb-2">
                        ...
                        <div v-if="can('recompensa-create')">
                            <router-link :to="{name: 'recompensas.create'}" class="btn btn-success">Nueva
                                recompensa</router-link>
                        </div>
                    </div>
                    <!-- :rowsPerPageOptions="[5, 10, 20, 50]" -->
                    <DataTable :value="recompensas.data" :first=index.value  paginator :rows="filas" tableStyle="min-width: 50rem"
                        ...
                        <Column header="Categorías" style="width: 25%" filterField="categorias.nombre"
                            ...
                            <template #body="slotProps">
                                <span v-for="cat in slotProps.data.categorias"
                                    class="ms-2 badge  bg-secondary bg-gradient">
                                    {{ cat.nombre }}
                                </span>
                            </template>
                        </Column>
                        ...
                        <Column style="width:100px;" class="pe-0 me-0">
                            <template #body="slotProps">
                                ...
                                <router-link v-if="can('recompensa-edit') && user.id == slotProps.data.usuario_id"
                                    :to="{ name: 'recompensas.edit', params: { id: slotProps.data.id } }"
                                ...
                                <span v-if="can('recompensa-delete') && user.id == slotProps.data.usuario_id"
                                    @click.prevent="deleteRecompensa(slotProps.data.id, slotProps.index)"
                        ...
                        <template #paginatorend>
                            <Dropdown appendTo="self" v-model="filas" :options="filasPorPagina" @change="changeFirst"/>
                        </template>
                    </DataTable>
                </div>
            </div>
...
<script setup>
    ...
    const filasPorPagina=[5, 10, 20, 50];
